  ...
  return isShowPicturePreview ? (
    <div className="ma5-imgbox PicturePreview">
      ...
      <div className="resources-action-bar">
        {ActionBody(Articel, Path, hidePicturePreview, Btn, rotate)}
      </div>
    </div>
  ) : null;
}
...
function ActionBody(Articel, Path, hidePicturePreview, Btn, rotate) {
  return (
    <div className="resources-action-bar__body">
      ...
      <div className="resources-action-bar__side-right">
        {Btn(
          "E-posta",
          `mailto:receperoglu1@hotmail.com?subject=${Articel}&body=${Path}`,
          "OutlookLogo"
        )}
        {Btn(
          "Whatsapp",
